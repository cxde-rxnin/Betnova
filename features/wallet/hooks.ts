...
export function useWithdraw() {
  ...
  return useMutation({
    mutationFn: async ({ amount, currency, destinationAddress }: { amount: number; currency: string; destinationAddress: string }) => {
      return await requestWithdrawal(amount, currency, destinationAddress);
    },
    onSuccess: () => {
